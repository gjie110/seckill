...
public class SeckillService {
    ...
    /** 秒杀下单主入口：限流 → 活动校验 → 库存扣减 → 发MQ（失败同步降级）。 */
    public String executeOrder(SeckillRequest request) {
        ...
        if (quantity > 3) throw new SeckillException(ResultCode.PURCHASE_LIMIT);
        ...
        if (!"regular".equals(ticketType.getChannel())) {
            ...
            if (cfg != null && cfg.getMaxPerUser() != null && cfg.getMaxPerUser() > 0) maxPerUser = cfg.getMaxPerUser();
    ...
    /** 滑动窗口限流：每分钟每个用户最多 N 次请求，超限抛限流异常。 */
    private void checkRateLimit(Long userId) {
        ...
        if (result == null || result == 0) throw new SeckillException(ResultCode.RATE_LIMIT);
    }
    ...
    private void checkPurchaseLimit(Long userId, Long productId, TicketType ticketType, int maxPerUser) {
        ...
        if (alreadyBought >= maxPerUser) throw new SeckillException(ResultCode.ALREADY_PURCHASED);
    }

    /** 执行 Redis Lua 脚本完成库存预扣，返回 1=成功 0=库存不足 -1=超限。 */
    private int doStockDeduction(Long userId, Long ticketTypeId, Integer quantity, int maxPerUser) {
        ...
        try {
            Long result = stringRedisTemplate.execute(
                    ...
                    String.valueOf(userId), String.valueOf(quantity), String.valueOf(maxPerUser)
            );
    ...
    private void sendOrderMessage(String orderNo, Long userId, Long productId, Long ticketTypeId,
        ...
        SeckillMessage message = SeckillMessage.builder()
                .orderNo(orderNo).userId(userId).productId(productId).ticketTypeId(ticketTypeId)
                .quantity(quantity).unitPrice(unitPrice).totalAmount(total).channel(channel)
    ...
    /** MQ 不可用时的同步降级：直接建订单入库，通过 doneKey 保证幂等。 */
    ...
    /** 回滚 Redis 预扣的库存和用户计数器（Lua 脚本失败时降级）。 */
    ...
    private void joinWaitlist(Long userId, Long productId, Long ticketTypeId, Integer quantity) {
        ...
        if (existingList != null && !existingList.isEmpty()) return;
